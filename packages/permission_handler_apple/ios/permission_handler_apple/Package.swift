...
let permissionDefines: [CSetting] = [
    ...
    // Nebu only reads the current Wi-Fi name while its setup UI is visible.
    // Compile the explicit foreground strategy so requestAlwaysAuthorization
    // is not linked into the app and no background-location purpose string is
    // required. Keep this deterministic across Xcode GUI, CLI and CI builds.
