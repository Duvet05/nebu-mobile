                ...
                SliverToBoxAdapter(
                  child: Column(
                    ...
                    children: [
                      ...
                      AuthBackButton(onPressed: () => context.canPop() ? context.pop() : context.go(AppRoutes.home.path)),
                      ...
                      Text(
                        ...
                        style: textTheme.titleMedium?.copyWith(color: context.colors.grey400),
                      ),
                      SizedBox(height: context.spacing.largePageBottomMargin),
                      if (authState.hasError && !authState.isLoading)
                        AuthErrorBanner(message: authState.error.toString().replaceFirst('Exception: ', '')),
                      SizedBox(height: context.spacing.titleBottomMargin),
                      AuthTextField(
                        controller: _identifierController,
                        label: 'auth.username_or_email'.tr(),
                        prefixIcon: Icons.person_outline_rounded,
                        validator: (v) => (v == null || v.isEmpty) ? 'auth.username_or_email_required'.tr() : null,
                      ),
                      SizedBox(height: context.spacing.titleBottomMargin),
                      AuthTextField(
                      ...
                      Align(
                        ...
                        child: TextButton(
                          onPressed: () => _showForgotPasswordDialog(context, ref, _identifierController.text),
                          child: Text('auth.forgot_password'.tr(), style: TextStyle(fontWeight: FontWeight.w600, color: context.theme.colorScheme.primary)),
                        ),
                      ),
                    ],
                  ),
...
class _SignUpLink extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        Text('${'auth.no_account'.tr()} ', style: TextStyle(color: context.colors.grey400)),
        GestureDetector(
          onTap: isLoading ? null : () => context.push(AppRoutes.signUp.path),
          child: Text(
            'auth.sign_up'.tr(),
            style: TextStyle(
              color: isLoading ? context.colors.grey500 : context.theme.colorScheme.primary,
